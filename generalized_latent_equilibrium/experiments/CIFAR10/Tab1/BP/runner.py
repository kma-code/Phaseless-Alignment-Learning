        ...
        for seed in seeds:
            ...
            if algo == 'PAL':
                    params.update(
                                            {
                            "rec_degs": True,                    
                            "bw_lr_factors": [0,0,0,0,0,0],
                            #"bw_lr_factors": [0,0,0,0,0,bw_lr],
                            #"regularizer": [0,0,alpha,0,alpha,alpha],
                            "regularizer": [0,0,0,0,0,0],
                            "tau_xi": [10,10,10,10,10,10],
                            "tau_HP": [10,10,10,10,10,10],
                            #"tau_LO": [1e+4,1e+4,1e+4,1e+4,1e+4,1e+4],
                            #"tau_LO": [1e+3,0,1e+3,0,1e+3,1e+3],
                            "tau_LO": [0,0,0,0,0,0],
                            #"sigma": [noise,0,noise,0,noise,0]
                            "sigma": [1e-2,0,1e-3,0,1e-3,0]
                            }
                    )
        ...
        if args.run and not args.gather:
                for i, params_per_lr in zip(range(len(params_arr)), params_arr):
                        for j, params in zip(range(len(params_per_lr)), params_per_lr):
                                ...
                                proc_name = ['sbatch', 'slurm_script_gpu.sh', 'python', 'experiments/CIFAR10/le_layers_cifar10_training.py', '--params', str(sim_dir / 'params.json')]
