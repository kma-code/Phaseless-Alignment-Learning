#!/usr/bin/env python
# PyTorch implementation of the Latent Equilibrium model. Batch-trainable version.
...
# the model.latent_equilibrium_torch.py version, which only supports training with single samples.
#
...
class LatentEqNetwork:
    ...
    def __init__(self, params: NetworkParams):
        self.device = torch.device('cuda' if torch.cuda.is_available() and params.on_cuda else 'cpu')
        ...
        self.st_input = torch.zeros((1, self.input_size), dtype=self.dtype, device=self.device)
        self.st_old_input = torch.zeros((1, self.input_size), dtype=self.dtype, device=self.device)
        self.st_target = torch.zeros((1, self.target_size), dtype=self.dtype, device=self.device)
        self.st_old_target = torch.zeros((1, self.target_size), dtype=self.dtype, device=self.device)
        ...
        self.old_x = torch.zeros((1, self.input_size), dtype=self.dtype, device=self.device)
        self.old_y = torch.zeros((1, self.target_size), dtype=self.dtype, device=self.device)
        self.dummy_label = torch.zeros((1, self.target_size), dtype=self.dtype, device=self.device)
        ...
        if params.check_with_profiler:
            ...
            self._profile_model(batch_size=1)
            # setup network
            self._initialize_network()
        ...
        # perform basic profiling
        if params.check_with_profiler:
            print("\nProfile model for 32 sample batch:\n--------------------------------")
            self._profile_model(batch_size=32)
            # setup network
            self._initialize_network()

        # perform mini-batch test run to get network performance stats
    ...
    def _initialize_network(self):
        ...
        self.st_voltage_lookaheads = torch.zeros((1, self.neuron_qty), dtype=self.dtype, device=self.device)  # initialize all voltage derivatives to 0
        self.st_input_current = torch.zeros((1, self.neuron_qty), dtype=self.dtype, device=self.device)
        self.st_error_current = torch.zeros((1, self.neuron_qty), dtype=self.dtype, device=self.device)
        self.st_basal_inputs = torch.zeros((1, self.neuron_qty), dtype=self.dtype, device=self.device)
        ...
        if self.feedback_alignment:
            self.st_feedback_weights = self._create_initial_weights(self.weight_mask,
                                                           self.arg_w_init_params['mean'],
                                                           self.arg_w_init_params['std'],
                                                           self.arg_w_init_params['clip'])
            self.st_feedback_weights = torch.tensor(self.st_feedback_weights.T, dtype=self.dtype, device=self.device)
        else:
            self.st_feedback_weights = torch.tensor(self.st_weights.t(), dtype=self.dtype, device=self.device)
        ...
        self.rho = torch.zeros((1, self.neuron_qty), dtype=self.dtype, device=self.device)                         # rates
        self.rho_deriv = torch.zeros((1, self.neuron_qty), dtype=self.dtype, device=self.device)                   # rate derivatives
        self.errors = torch.zeros((1, self.neuron_qty), dtype=self.dtype, device=self.device)                      # errors

        self.noise = torch.zeros((1, self.neuron_qty), dtype=self.dtype, device=self.device)                       # noise
    ...
    def _profile_model(self, batch_size=32):
        ...
        _, cpu_profiler, cuda_profiler = bottleneck(lambda: [self.update_network(sample_input, sample_output, True) for _ in range(100)])
    ...
    def report_simulation_params(self):
        ...
        print('Feedback alignment: ', self.feedback_alignment)
    ...
    def _perform_update_step(self, train_W=False):
        ...
        # voltages = torch.zeros((1, self.neuron_qty), dtype=self.dtype)
        # basal_inputs = torch.zeros((1, self.neuron_qty), dtype=self.dtype)
        # lookahead_voltages = torch.zeros((1, self.neuron_qty), dtype=self.dtype)
        # voltages_deriv = torch.zeros((1, self.neuron_qty), dtype=self.dtype)
        # layerwise_syn_current = torch.zeros((1, self.neuron_qty), dtype=self.dtype)

        # PERFORM SYNAPTIC LPF FOR RATES & ERRORS
        if self.tau_s != 0:
            dot_input = (self.rho - self.st_input_current) / self.tau_s
            self.st_input_current += self.dt * dot_input
            dot_error = (self.errors - self.st_error_current) / self.tau_s
            self.st_error_current += self.dt * dot_error
        else:
            self.st_input_current = self.rho.detach().clone()
            self.st_error_current = self.errors.detach().clone()

        # select chosen model variant
        if self.model_variant == ModelVariant.VANILLA:
            # vanilla implementation of LE
            basal_inputs = self._calculate_basal_inputs(self.st_input_current, self.st_weights, self.st_biases)  # W * r + b
            voltages_deriv = 1.0 / self.tau * (basal_inputs - self.st_voltages + self.st_error_current)
            ...
            voltages = torch.zeros([self.st_input.shape[0], self.neuron_qty], dtype=self.dtype, device=self.device)
            basal_inputs = torch.zeros([self.st_input.shape[0], self.neuron_qty], dtype=self.dtype, device=self.device)
            lookahead_voltages = torch.zeros([self.st_input.shape[0], self.neuron_qty], dtype=self.dtype, device=self.device)
            voltages_deriv = torch.zeros([self.st_input.shape[0], self.neuron_qty], dtype=self.dtype, device=self.device)
            layerwise_syn_current = torch.zeros([self.st_input.shape[0], self.neuron_qty], dtype=self.dtype, device=self.device)

            # calculate forward pathway similar to deep learning
            # and add errors in between
            layerwise_inputs = torch.zeros([self.st_input.shape[0], self.neuron_qty], dtype=self.dtype, device=self.device)
            for layer in range(len(self.layers)):
                # calculate layerwise input
                if layer == 0:
                    layerwise_inputs[:, :self.input_size] = self.st_input  # external input
                else:
                    layerwise_inputs = torch.matmul(self.st_weights, layerwise_syn_current.t()).t()  # internal input (W_i * r) + b
        ...
        zeros = torch.zeros((1, self.neuron_qty), dtype=self.dtype, device=self.device)
        if self.arg_noise_width != 0:
            # define Wiener process
            self.noise = np.sqrt(self.dt) * torch.normal(mean=zeros, std=self.arg_noise_width)
        else:
            self.noise = zeros
        ...
        self.rho += self.noise
        # CALCULATE ERRORS
        # errors = torch.zeros((1, self.neuron_qty), dtype=self.dtype)
        ...
        if train_W:
            dot_weights = self.get_weight_derivatives().mean(0).squeeze()
        ...
        if self.use_biases:  # if we use biases
            if train_W:
                dot_biases = self.get_bias_derivatives().mean(0)  # do bias update
    ...
    def _adapt_parallel_network_qty(self):
        ...
        """
        batch_size = self.st_input.shape[0]
        if len(self.st_voltages.shape) != 2 or self.st_voltages.shape[0] != batch_size:
            ...
            self.st_voltage_lookaheads = torch.repeat_interleave(self.st_voltage_lookaheads, repetition_vector, dim=0).clone()
            self.st_input_current = torch.repeat_interleave(self.st_input_current, repetition_vector, dim=0).clone()
            self.st_error_current = torch.repeat_interleave(self.st_error_current, repetition_vector, dim=0).clone()
            self.rho = torch.repeat_interleave(self.rho, repetition_vector, dim=0).clone()
            self.rho_deriv = torch.repeat_interleave(self.rho_deriv, repetition_vector, dim=0).clone()
    ...
    def set_input_and_target(self, input, target):
        ...
        if len(input.shape) == 2:
            ...
            batch_size = 1
            input = input.unsqueeze(0).expand((batch_size, -1)).clone()
            target = target.unsqueeze(0).expand((batch_size, -1)).clone()

        if len(self.st_input.shape) != 2:
            self.st_input = self.st_input.unsqueeze(0)

        if len(self.st_target.shape) != 2:
            self.st_target = self.st_target.unsqueeze(0)

        if self.st_input.shape[0] != batch_size:
            self.st_input = self.st_input.mean(0).expand((batch_size, -1)).clone()

        if self.st_target.shape[0] != batch_size:  # new input batch size is unequal the previous batch size
            self.st_target = self.st_target.mean(0).expand((batch_size, -1)).clone()
    ...
    def get_weight_derivatives(self):
        ...
        """
        return torch.einsum('bi,bj->bij', self.errors, self.rho) * self.weight_mask * self.arg_lrate_W
    ...
    def _calculate_weight_derivatives(self, voltage_lookaheads, weights, biases):
        ...
        return torch.einsum('bi,bj->bij', voltage_lookaheads - basal_inputs, rho) * self.weight_mask * self.arg_lrate_W
    ...
    def _calculate_basal_inputs(self, rho, weights, biases):
        ...
        """
        basal_input = torch.matmul(weights, rho.t()).t() + biases  # internal input (W_i * r) + b
        ...
        return torch.cat((self.st_input, basal_input[:, self.input_size:]), dim=1)  # internal input + external input = total input
    ...
    def _calculate_errors(self, voltages, voltage_lookaheads, rho, rho_deriv, target, basal_inputs):
        """
        Calculate:
            layerwise error:    e = diag(r') B (U - Wr) + β e^{trg}
        ...
        """
        # e (missing e_{trg})
        err = rho_deriv * torch.matmul(self.st_feedback_weights, (voltage_lookaheads - basal_inputs).t()).t()
    ...
    def _calculate_e_trg(self, target, voltages, voltage_lookaheads, rho, rho_deriv):
        ...
        if self.target_type == TargetType.RATE:
            e_trg = torch.mul(rho_deriv[:, -self.target_size:], (target - rho[:, -self.target_size:]))
    ...
    def fit_batch(self, x=None, y=None, n_updates: int=100, batch_iteration=-1, batch_qty=-1, verbose: int=1, is_timeseries=False):
        ...
        for update_i in range(n_updates):
            ...
            self.update_network(samples, labels, train_W=True)
    ...
    def predict(self, x=None, n_updates: int = 100, batch_size=1, verbose=1, is_timeseries=False):
        ...
        for batch_i, (x, _) in enumerate(data_loader):
            ...
            batch_predictions = self.predict_batch(x, batch_iteration=batch_i, batch_qty=batch_qty, n_updates=n_updates, verbose=verbose, is_timeseries=is_timeseries)
    ...
    def predict_batch(self, x, n_updates: int=100, batch_iteration=-1, batch_qty=-1, verbose: int=1, is_timeseries: bool=False):
        ...
        for update_i in range(n_updates):
            ...
            self.update_network(samples, self.dummy_label, train_W=False)
        ...
        if self.target_type == TargetType.RATE:
            ...
            batch_predictions = rates[:, -self.target_size:].detach().cpu().numpy()
        else:
            volts = self.st_voltage_lookaheads
            batch_predictions = volts[:, -self.target_size:].detach().cpu().numpy()
